...
const HOOK = '\nwindow.__test = { setBoss:(b)=>{ boss = b; }, wp:(b)=>weakPoints(b), P:()=>p, anchors:()=>anchors, reset:()=>reset(), setShield:(v)=>{ shield = v; }, setChain:(v)=>{ chain = v; chainTimer = 0; }, pushShot:(s)=>{ shots.push(s); }, dmg:(r,m)=>{ damageBoss(r,m); }, lifeStats:()=>({ boss:lifeBossKills, kraken:lifeKrakenKills }) };\n';
